...
final class ClassUtils {
  ...
  private static final HashMap<Class, Class> COMPLEX_WRAPPERS = new HashMap<>();
  ...
  static {
    ...
    COMPLEX_WRAPPERS.put(DevxitUseCaseCallback.class, DevxitUseCaseCallback.class);
  }
  ...
  static boolean canAssign(Class<?> baseClass, Class<?> classToCheck) {
    if (baseClass.isAssignableFrom(classToCheck)) {
      ...
      if (baseClass.isPrimitive()) {
        ...
        Class complexClass = COMPLEX_WRAPPERS.get(classToCheck);
        if (complexClass != null) {
          return complexClass.isAssignableFrom(classToCheck);
        }
      }
    }
