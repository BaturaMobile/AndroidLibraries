...
class IDCardValidator private constructor(){
    ...
    init{
        countries.put(IDCardValidator.COUNTRIES.ES, Regex("^[0-9]{8}[A-Za-z]\$"))
    ...
    private fun isSpanishCardValid(codeCard: String) : Boolean {
        if(countries[COUNTRIES.ES]!!.matches(codeCard)){
            val onlyNumber = Integer.valueOf(codeCard.substring(0,8))
            val remainder = onlyNumber % 23
            if (spanishLetterVerefied.containsKey(remainder) && spanishLetterVerefied[remainder] == (codeCard[8].toString())){
                return true
            }
        }
        return false
